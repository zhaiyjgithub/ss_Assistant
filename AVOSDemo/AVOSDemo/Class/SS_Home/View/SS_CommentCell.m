...
- (void)setCommentModel:(SS_DetailOfStoreModel *)commentModel
{
    ...
    //使用新方法计算文字大小
    CGSize textSize = [commentModel.instruction sizeWithFont:[UIFont systemFontOfSize:13.0] maxSize:CGSizeMake(300, MAXFLOAT)];
    
    NSLog(@"计算得到的宽高:%f --- %f",textSize.width,textSize.height);
    //设置label的大小以及cell的高度
    self.commentLabel.frame = CGRectMake(self.frame.origin.x+10, self.frame.origin.y +20, textSize.width, textSize.height);
    //获取这个cell的高度
    self.commentCellHeight = textSize.height+40;
}
